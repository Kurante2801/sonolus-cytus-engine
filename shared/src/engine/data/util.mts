...
export function getGameplayBounds(
	screenWidth: number,
	screenHeight: number,
	...
	const verticalRatio =
		1 - (screenWidth * (topRatio + bottomRatio)) / screenHeight + (3 - verticalMargin) * 0.05;
	const verticalOffset = -(screenWidth * (topRatio - (topRatio + bottomRatio) / 2));

	const chartToScreenX = (x: number): number =>
		(x * 2 * horizontalRatio - horizontalRatio) * screenWidth * 0.5;
